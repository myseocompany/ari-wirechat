...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
  <div class="flex flex-col gap-1">
    <h1 class="mb-1">Simulador SPIN — Entrenamiento</h1>
...
<div class="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
  <form action="/reports/views/simulador-spin" method="GET" class="flex flex-wrap items-end gap-4">
    <div class="flex flex-col gap-1">
    ...
    <div class="flex flex-col gap-1">
      ...
      <select name="escenario" class="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-slate-400 focus:outline-none">
        ...
        <option value="emprendedor_nuevo" @selected($request->escenario === 'emprendedor_nuevo')>Emprendedor</option>
      </select>
    </div>
    <div class="flex flex-col gap-1">
      <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Puntaje mínimo</label>
...
<div class="space-y-4">
  @forelse ($model as $item)
    @php
      ...
      $colorCard  = $puntaje >= 7 ? 'border-emerald-200' : ($puntaje >= 5 ? 'border-amber-200' : ($puntaje > 0 ? 'border-red-200' : 'border-slate-200'));
      $colorBadge = $puntaje >= 7 ? 'bg-emerald-100 text-emerald-700' : ($puntaje >= 5 ? 'bg-amber-100 text-amber-700' : ($puntaje > 0 ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-500'));
    ...
    <div class="rounded-2xl border {{ $colorCard }} bg-white p-5 shadow-sm">
      ...
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="flex flex-wrap items-center gap-3">
          @if ($puntaje > 0)
            <span class="inline-flex items-center rounded-full px-3 py-1 text-lg font-bold {{ $colorBadge }}">{{ $puntaje }}/10</span>
          @else
