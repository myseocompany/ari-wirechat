...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
  <div class="flex flex-col gap-1">
    <h1 class="mb-1">SPIN Simulator — Training Sessions</h1>
    <div class="text-xs text-slate-500">{{ $model->total() }} sessions recorded</div>
  </div>
</div>
...
<div class="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
  <form action="/reports/views/spin-simulator" method="GET" class="flex flex-wrap items-end gap-4">
    <div class="flex flex-col gap-1">
      <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">From</label>
    ...
    <div class="flex flex-col gap-1">
      <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">To</label>
    ...
    <div class="flex flex-col gap-1">
      <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Scenario</label>
      <select name="escenario" class="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-slate-400 focus:outline-none">
        ...
        <option value="productor_pereira" @selected($request->escenario === 'productor_pereira')>Producer (Pereira)</option>
        <option value="restaurantero_medellin" @selected($request->escenario === 'restaurantero_medellin')>Restaurant owner (Medellín)</option>
        <option value="inversionista_bogota" @selected($request->escenario === 'inversionista_bogota')>Investor (Bogotá)</option>
        <option value="emprendedor_nuevo" @selected($request->escenario === 'emprendedor_nuevo')>New entrepreneur</option>
      </select>
    </div>
    <div class="flex flex-col gap-1">
      <label class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">Min. score</label>
...
@php
  $spinFields = [
    'hizo_apertura_correcta'  => 'Opening',
    'preguntas_situacion'     => 'Situation',
    'identifico_problema'     => 'Problem',
    'hizo_implicacion'        => 'Implication',
    'cliente_dijo_beneficio'  => 'Need-payoff',
    'cerro_con_paso_concreto' => 'Close',
  ];
@endphp
    ...
    <div class="rounded-2xl border {{ $colorCard }} bg-white p-5 shadow-sm">
      ...
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="flex flex-wrap items-center gap-3">
          ...
          @else
            <span class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-400">No score</span>
          @endif
        ...
        <div class="mt-4 grid gap-3 text-sm lg:grid-cols-3">
          @if ($item->resumen_llamada)
            <div class="rounded-xl bg-slate-50 p-3">
              <div class="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Summary</div>
              <div class="text-slate-700">{{ $item->resumen_llamada }}</div>
            </div>
          @endif
          @if ($item->principal_error)
            <div class="rounded-xl bg-red-50 p-3">
              <div class="mb-1 text-xs font-semibold uppercase tracking-wide text-red-400">Main error</div>
              <div class="text-red-700">{{ $item->principal_error }}</div>
            </div>
          @endif
          @if ($item->recomendacion)
            <div class="rounded-xl bg-amber-50 p-3">
              <div class="mb-1 text-xs font-semibold uppercase tracking-wide text-amber-500">Recommendation</div>
      ...
      @if ($transcript)
        <details class="mt-3">
          <summary class="cursor-pointer text-xs text-slate-400 hover:text-slate-600">View transcript</summary>
