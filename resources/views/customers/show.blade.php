              ...
              @if(app_feature_enabled('customer_call_briefing_enabled', false))
                <section class="rounded-lg border border-slate-200 bg-slate-50 p-3" aria-labelledby="call-briefing-title">
                  ...
                  @elseif($callBriefing?->status === 'failed')
                    <p class="mt-2 text-xs text-red-700">No pudimos preparar la llamada.</p>
                  @elseif($callBriefing?->status === 'insufficient_context')
                    <p class="mt-2 text-xs text-amber-800">No hay suficiente historial verificable para preparar esta llamada.</p>
                  @elseif(!$callBriefing)
                    <p class="mt-2 text-xs text-slate-600">Genera un resumen antes de llamar para retomar el contexto del cliente.</p>
                  @endif

                  @if($callBriefing?->status === 'ready')
                    @if($callBriefingStale)
                      <p class="mt-2 rounded-md border border-amber-300 bg-amber-50 px-2 py-1 text-xs text-amber-900">Resumen desactualizado.</p>
                    @endif
                    <div class="mt-3 space-y-3 text-xs text-slate-700">
                      <div>
                        <h4 class="font-semibold text-slate-900">Resumen</h4>
                        <p class="mt-1 whitespace-pre-line">{{ $callBriefing->summary }}</p>
                      </div>
                      <div>
                        <h4 class="font-semibold text-slate-900">Lo que ya sabemos</h4>
                        <ul class="mt-1 space-y-1">
                          @forelse($callBriefing->known_facts ?? [] as $fact)
                            <li>• {{ $fact['text'] }} <span class="text-slate-500">({{ $fact['occurred_at'] ?? $fact['source'] }})</span></li>
                          @empty
                      ...
                      <div>
                        <h4 class="font-semibold text-slate-900">Datos por confirmar</h4>
                        <ul class="mt-1 space-y-1">
                          @forelse($callBriefing->conflicting_facts ?? [] as $fact)
                            <li>• {{ $fact['text'] }} <span class="text-slate-500">({{ implode(', ', $fact['sources']) }})</span></li>
                          @empty
                      ...
                      <div>
                        <h4 class="font-semibold text-slate-900">Evita preguntar de nuevo</h4>
                        <ul class="mt-1 space-y-1">
                          @forelse($callBriefing->avoid_asking ?? [] as $item)
                            <li>• {{ $item['text'] }} <span class="text-slate-500">({{ $item['reason'] }} · {{ $item['source'] }})</span></li>
                          @empty
                      ...
                      <div>
                        <h4 class="font-semibold text-slate-900">Apertura sugerida</h4>
                        <p class="mt-1">{{ $callBriefing->suggested_opening }}</p>
                        <h4 class="mt-2 font-semibold text-slate-900">Siguiente pregunta</h4>
                        <p class="mt-1">{{ $callBriefing->next_question }}</p>
                      </div>
                    </div>
                    <p class="mt-3 text-xs text-slate-500">Generado {{ $callBriefing->generated_at?->format('Y-m-d H:i') }} · {{ $callBriefing->model }}</p>
                  @endif
                </section>
              @endif
