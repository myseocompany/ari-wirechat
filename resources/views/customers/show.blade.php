...
@if(app_feature_enabled('customer_call_briefing_enabled', false))
  <section class="mb-4 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm text-slate-800" aria-labelledby="call-briefing-title">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h2 id="call-briefing-title" class="text-base font-semibold text-violet-950">Preparación de llamada</h2>
        @if($callBriefing?->status === 'pending')
          <p class="mt-1 text-violet-800">Estamos revisando el historial del cliente.</p>
          @if($callBriefing->updated_at?->lessThan(now()->subMinutes(3)))
            <p class="mt-1 text-amber-800">La preparación tardó más de lo esperado.</p>
          @endif
        @elseif($callBriefing?->status === 'failed')
          <p class="mt-1 text-red-700">No pudimos preparar la llamada.</p>
        @elseif($callBriefing?->status === 'insufficient_context')
          <p class="mt-1 text-amber-800">No hay suficiente historial verificable para preparar esta llamada.</p>
        @elseif(!$callBriefing)
          <p class="mt-1 text-slate-600">Todavía no hay una preparación para este cliente.</p>
        @endif
      </div>
      @if($callBriefing?->status !== 'pending' || $callBriefing->updated_at?->lessThan(now()->subMinutes(3)))
        <form method="POST" action="{{ route('customers.call-briefing.store', $model) }}">
          ...
          <button type="submit" class="inline-flex items-center rounded-md bg-violet-700 px-3 py-2 text-xs font-semibold text-white transition hover:bg-violet-800">
            {{ $callBriefing?->status === 'pending' ? 'Reintentar' : ($callBriefing ? ($callBriefingStale ? 'Actualizar preparación' : 'Preparar de nuevo') : 'Preparar llamada') }}
          </button>
        </form>
      @endif
    </div>

    @if($callBriefing?->status === 'ready')
      @if($callBriefingStale)
        <p class="mt-3 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-amber-900">Resumen desactualizado: hay información nueva en el historial.</p>
      @endif
      <div class="mt-4 grid gap-4 lg:grid-cols-2">
        <div class="rounded-lg bg-white p-3 shadow-sm lg:col-span-2">
          <h3 class="font-semibold text-slate-900">Resumen</h3>
          <p class="mt-1 whitespace-pre-line text-slate-700">{{ $callBriefing->summary }}</p>
        </div>
        <div class="rounded-lg bg-white p-3 shadow-sm">
          <h3 class="font-semibold text-slate-900">Lo que ya sabemos</h3>
          <ul class="mt-2 space-y-1 text-slate-700">
            @forelse($callBriefing->known_facts ?? [] as $fact)
              <li>• {{ $fact['text'] }} <span class="text-xs text-slate-500">({{ $fact['occurred_at'] ?? $fact['source'] }})</span></li>
            @empty
              <li class="text-slate-500">Sin hechos destacados.</li>
            @endforelse
          </ul>
        </div>
        <div class="rounded-lg bg-white p-3 shadow-sm">
          <h3 class="font-semibold text-slate-900">Datos por confirmar</h3>
          <ul class="mt-2 space-y-1 text-slate-700">
            @forelse($callBriefing->conflicting_facts ?? [] as $fact)
              <li>• {{ $fact['text'] }} <span class="text-xs text-slate-500">({{ implode(', ', $fact['sources']) }})</span></li>
            @empty
              <li class="text-slate-500">Sin contradicciones destacadas.</li>
            @endforelse
          </ul>
        </div>
        <div class="rounded-lg bg-white p-3 shadow-sm">
          <h3 class="font-semibold text-slate-900">Evita preguntar de nuevo</h3>
          <ul class="mt-2 space-y-1 text-slate-700">
            @forelse($callBriefing->avoid_asking ?? [] as $item)
              <li>• {{ $item['text'] }} <span class="text-xs text-slate-500">({{ $item['reason'] }} · {{ $item['source'] }})</span></li>
            @empty
              <li class="text-slate-500">Sin advertencias.</li>
            @endforelse
          </ul>
        </div>
        <div class="rounded-lg bg-white p-3 shadow-sm">
          <h3 class="font-semibold text-slate-900">Apertura sugerida</h3>
          <p class="mt-2 text-slate-700">{{ $callBriefing->suggested_opening }}</p>
          <h3 class="mt-3 font-semibold text-slate-900">Siguiente pregunta</h3>
          <p class="mt-2 text-slate-700">{{ $callBriefing->next_question }}</p>
        </div>
      </div>
      <p class="mt-3 text-xs text-slate-500">Generado {{ $callBriefing->generated_at?->format('Y-m-d H:i') }} · {{ $callBriefing->model }} · {{ $callBriefing->prompt_version }}</p>
    @endif
  </section>
@endif
