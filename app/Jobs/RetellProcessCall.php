...
class RetellProcessCall implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // 4) Crear/actualizar Acción (idempotente por retell_call_id único)
        $recordingUrl = $this->normalizeUrl($recordingUrl);
        $publicLogUrl = $this->normalizeUrl($publicLogUrl);

        $action = Action::firstOrNew(['retell_call_id' => $callId]);

        if (! $action->exists) {
            $action->customer_id = $customer->id;
            $action->type_id = 104;
            $action->creator_user_id = 1;
        }
        ...
        // Priorizar recording_url cuando llegue en eventos posteriores.
        if ($recordingUrl !== null) {
            $action->url = $recordingUrl;
        } elseif (blank($action->url) && $publicLogUrl !== null) {
            $action->url = $publicLogUrl;
        }

        $currentNote = (string) ($action->note ?? '');
        if ($currentNote === '' || mb_strlen($note) > mb_strlen($currentNote)) {
            $action->note = $note;
        }
