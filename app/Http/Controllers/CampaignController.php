...
class CampaignController extends Controller
{
    ...
    {
        $campaign = Campaign::with('customer_meta_data')->findOrFail($id);
        $questions = $campaign->customer_meta_data()->orderBy('id')->get();
        $questionIds = $questions->pluck('id')->filter()->values();

        $responses = collect();
        if ($questionIds->isNotEmpty()) {
            $responses = CustomerMeta::query()
                ->leftJoin('customers', 'customers.id', '=', 'customer_metas.customer_id')
                ->leftJoin('customer_meta_datas', 'customer_meta_datas.id', '=', 'customer_metas.meta_data_id')
                ->whereIn('customer_metas.meta_data_id', $questionIds)
                ->orderByDesc('customer_metas.id')
                ->get([
                    'customer_metas.id',
                    'customer_metas.customer_id as customer_id',
                    'customer_metas.meta_data_id',
                    'customer_metas.value',
                    'customer_metas.created_at',
                    'customers.name',
                    'customers.phone',
                    'customers.phone2',
                    'customer_meta_datas.value as question_label',
                ]);
        }

        $responsesByCustomer = $responses
            ->groupBy('customer_id')
            ->map(function ($items) {
                $customer = $items->first();
                $answers = $items
                    ->groupBy('meta_data_id')
                    ->map(function ($metaItems) {
        ...
        return view('campaigns.show', [
            'campaign' => $campaign,
            'questions' => $questions,
            'responsesByCustomer' => $responsesByCustomer,
        ]);
    }
...
(now()-max(if(outbound=0, actions.created_at, null))) DESC')
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($request->message as $key => $value) {
            ...
            if ($message == null) {
                $data[] = ['audience_id' => $id, 'name' => $value];
            }
        }
        $model->save();
        Message::insert($data);
