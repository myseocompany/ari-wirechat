...
class OrderController extends Controller
{
                ...
                function ($query) use ($request) {
                    ...
                    if (isset($request->category_id) && ($request->category_id != null)) {
                        if ($request->category_id == 1) {
                            $query = $query->whereIn('products.category_id', [4, 5, 6]);
                        } elseif ($request->category_id == 2) {
                            $query = $query->whereIn('products.category_id', [1, 2, 3]);
                        }
                    }
                }
            )
            ...
            ->where(
                ...
                function ($query) use ($request) {
                    if (isset($request->week)) {
                        ...
                        // dd($from_date);
                        $query->whereBetween('orders.created_at', [$from_date, $to_date]);
                    } elseif (isset($request->from_date) && ($request->from_date != null)) {
                        $query->whereBetween('orders.updated_at', [$request->from_date, $request->to_date]);
                    } else {
    ...
    public function storeQuick(StoreQuickOrderRequest $request): RedirectResponse
    {
        ...
        if ($product->country && strtoupper((string) $product->country) !== strtoupper($request->country)) {
            return back()
                ->withErrors(['product_id' => 'El producto no pertenece al país seleccionado.'], 'quickOrder')
                ->withInput();
        }

        $statusId = OrderStatus::query()->orderBy('id')->value('id');

        $order = new Order;
        $order->customer_id = $customer->id;
        $order->billing_name = $customer->name;
        $order->billing_document = $customer->document;
        $order->billing_phone = $customer->phone;
        $order->billing_phone2 = $customer->phone2;
        ...
        $order->updated_user_id = Auth::id();
        $order->status_id = $statusId;
        $order->user_id = $customer->user_id;
        $order->save();

        $price = $product->total ?? $product->price ?? 0;
