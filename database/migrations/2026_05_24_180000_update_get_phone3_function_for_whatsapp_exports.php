...
BEGIN
    ...
    SET candidate = formatPhone(phone);
    IF candidate IS NOT NULL THEN
        SET digits = REGEXP_REPLACE(candidate, '[^0-9]', '');
        IF digits REGEXP '^(57)?3[0-9]{9}$' THEN
            RETURN RIGHT(digits, 10);
        END IF;
    END IF;

    SET candidate = formatPhone(phone2);
    IF candidate IS NOT NULL THEN
        SET digits = REGEXP_REPLACE(candidate, '[^0-9]', '');
        IF digits REGEXP '^(57)?3[0-9]{9}$' THEN
            RETURN RIGHT(digits, 10);
    ...
    SET candidate = formatPhone(contact_phone2);
    IF candidate IS NOT NULL THEN
        SET digits = REGEXP_REPLACE(candidate, '[^0-9]', '');
        IF digits REGEXP '^(57)?3[0-9]{9}$' THEN
            RETURN RIGHT(digits, 10);
